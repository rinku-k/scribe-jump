defmodule SocialScribeWeb.SalesforceModalMoxTest do
  ...
  describe "Salesforce Modal with mocked API" do
    ...
    test "selecting contact triggers suggestion generation", %{conn: conn, meeting: meeting} do
      ...
      # The suggestion generation goes through AIContentGeneratorApi (mocked)
      # Allow calls from any process since LiveView runs in a spawned process
      SocialScribe.AIContentGeneratorMock
      |> stub(:generate_salesforce_suggestions, fn _meeting ->
        {:ok, mock_suggestions}
      end)
      ...
      # Select the contact - this triggers async suggestion generation
      view
      |> element("button[phx-click='select_contact'][phx-value-id='sf_123']")
      |> render_click()

      :timer.sleep(500)

      # After selecting contact, modal should still be present
  ...
  describe "Salesforce API behavior delegation" do
    ...
    test "search_contacts delegates to implementation", %{credential: credential} do
      expected = [%{id: "sf_1", firstname: "Test", lastname: "User"}]
    ...
    test "get_contact delegates to implementation", %{credential: credential} do
      expected = %{id: "sf_123", firstname: "John", lastname: "Doe"}

      SocialScribe.SalesforceApiMock
      |> expect(:get_contact, fn _cred, contact_id ->
        assert contact_id == "sf_123"
        {:ok, expected}
      end)

      assert {:ok, ^expected} =
               SocialScribe.SalesforceApiBehaviour.get_contact(credential, "sf_123")
    end

    test "update_contact delegates to implementation", %{credential: credential} do
      updates = %{"Phone" => "555-1234", "Title" => "CTO"}
      expected = %{id: "sf_123", phone: "555-1234", jobtitle: "CTO"}
      ...
      assert {:ok, ^expected} =
               SocialScribe.SalesforceApiBehaviour.update_contact(credential, "sf_123", updates)
    end
